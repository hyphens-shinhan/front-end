...
interface MentorApplicationModalContentProps {
  mentor: Mentor
}

/**
 * 멘토링 요청 폼 (hyphens-frontend MentorProfile request form과 동일한 구조).
 * 날짜 → 시간 → 만남 방식 → 메시지(선택) → 요청 보내기 / 취소
 */
export function MentorApplicationModalContent({
  mentor,
}: MentorApplicationModalContentProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // TODO: API 연동 (mentorship_requests 또는 bookAppointment)
